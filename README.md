...
открывает панель поверх живого чата. Каждый кейс можно запустить отдельно;
в пошаговом режиме выполнение встаёт на контрольных точках (`ctx.checkpoint`)
до клика «Далее» — а сценарий при этом двигает настоящий видимый стор, так
что каждый шаг рендерится в чате за панелью. Это не дублированные «живые»
тесты: те же самые кейсы, контекст (`ctx.useTransport`/`ctx.checkpoint`)
в Node — no-op и свежий стор, в браузере — паузы и видимый синглтон.
Запуск сбрасывает текущий диалог (`reset()`); по окончании прогона настоящий
